...
#ifdef LB_BOUNDARIES
  double* boundary_forces = (double*) pmalloc(3*n_lb_boundaries*sizeof(double));
...
void lb_calc_densprof(double *result, int *params) {
  ...
  if (this_node !=0) params = (int*) pmalloc(3*sizeof(int));
  ...
  if (this_node == newroot)
    result = (double*) prealloc(result,box_l[pdir]/lblattice.agrid*sizeof(double));

  if (involved) {

    profile = (double*) pmalloc(lblattice.grid[pdir]*sizeof(double));
...
void lb_calc_velprof(double *result, int *params) {
  ...
  if (this_node != 0) params = (int*) pmalloc(4*sizeof(int));
  ...
  if (involved) {
    ...
    velprof = (double*) pmalloc(lblattice.grid[pdir]*sizeof(double));
