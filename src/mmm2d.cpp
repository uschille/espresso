...
double MMM2D_add_far(int f, int e)
{
  ...
  undone = (int*)pmalloc((n_scxcache + 1)*sizeof(int));
...
void MMM2D_on_resort_particles()
{
  ...
  if (cell_structure.type == CELL_STRUCTURE_LAYERED) {
    ...
    scxcache = (SCCache*)prealloc(scxcache, n_scxcache*n_localpart*sizeof(SCCache));
    scycache = (SCCache*)prealloc(scycache, n_scycache*n_localpart*sizeof(SCCache));
    
    partblk   = (double*)prealloc(partblk,  n_localpart*8*sizeof(double));
    lclcblk   = (double*)prealloc(lclcblk,  n_cells*8*sizeof(double));
    gblcblk   = (double*)prealloc(gblcblk,  n_layers*8*sizeof(double));
  }
  MMM2D_self_energy();
}
