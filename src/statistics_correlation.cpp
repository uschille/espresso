...
//  }
  ...
  self->A_data = (double*)pmalloc((tau_lin+1)*hierarchy_depth*dim_A*sizeof(double));
  if (self->autocorrelation) 
    self->B_data = self->A_data;
  else 
    self->B_data = (double*)pmalloc((tau_lin+1)*hierarchy_depth*dim_B*sizeof(double));
  
  self->n_data=0;
  self->A_accumulated_average = (double*)pmalloc(dim_A*sizeof(double));
  self->A_accumulated_variance= (double*)pmalloc(dim_A*sizeof(double));
  for (k=0; k<dim_A; k++) {
  ...
  if (self->autocorrelation) {
    ...
    self->B_accumulated_average = (double*)pmalloc(dim_B*sizeof(double));
    self->B_accumulated_variance = (double*)pmalloc(dim_B*sizeof(double));
    for (k=0; k<dim_B; k++) {
  ...
  self->tau = (int*)                pmalloc(self->n_result*sizeof(int));
  self->n_sweeps = (unsigned int*)  pmalloc(self->n_result*sizeof(int));
  self->result_data  = (double*)    pmalloc(self->n_result*dim_corr*sizeof(double));
  self->n_vals = (unsigned int*) pmalloc(hierarchy_depth*sizeof(unsigned int));

  // allocate space for convenience pointer to A and B buffers
  self->A = (double***)pmalloc(hierarchy_depth*sizeof(double**));
  if(self->autocorrelation) self->B = self->A;
  else self->B = (double***)pmalloc(hierarchy_depth*sizeof(double**));

  for (i=0; i<self->hierarchy_depth; i++) {
    self->A[i] = (double**) pmalloc((self->tau_lin+1)*sizeof(double*));
    if(!self->autocorrelation) self->B[i] = (double**) pmalloc((self->tau_lin+1)*sizeof(double*));
  }
  ...
  self->result  = (double**)        pmalloc(self->n_result*sizeof(double*));
  ...
  self->newest = (unsigned int *)pmalloc(hierarchy_depth*sizeof(unsigned int));
  for ( i = 0; i<self->hierarchy_depth; i++ ) {
...
//(self->newest[i]+2) % (self->tau_lin+1), i+1, self->newest[i+1], self->n_vals[i]);
  ...
  double* temp = (double*)pmalloc(self->dim_corr*sizeof(double));
  if (!temp)
    return 4;
