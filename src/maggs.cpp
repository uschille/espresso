...
void maggs_setup_local_lattice()
{
  ...
  lattice  = (t_site*) pmalloc(xyzcube*sizeof(t_site));
  neighbor = (t_dirs*) pmalloc(xyzcube*sizeof(t_dirs));
	
  Bfield   = (double*) pmalloc(3*xyzcube*sizeof(double));
  Dfield   = (double*) pmalloc(3*xyzcube*sizeof(double));
...
{ 
  ...
  if(Npart>Npart_old) {
    grad = (double *) prealloc(grad, 12*Npart*sizeof(double));
    Npart_old = Npart;
  }
