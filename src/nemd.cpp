...
void nemd_init(int n_slabs, int n_exchange, double shear_rate) 
{
  ...
  nemddata.slab             = (Slab *)pmalloc(nemddata.n_slabs*sizeof(Slab));
  nemddata.velocity_profile = (double *)pmalloc(nemddata.n_slabs*sizeof(double));
  ...
  if(nemddata.n_exchange > 0) {
    nemddata.slab[nemddata.top_slab].fastest = (int *)pmalloc(nemddata.n_exchange*sizeof(int));
    nemddata.slab[nemddata.mid_slab].fastest = (int *)pmalloc(nemddata.n_exchange*sizeof(int));
  }
