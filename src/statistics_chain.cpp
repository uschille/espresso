...
void calc_re(double **_re)
{
  ...
  double *re=NULL, tmp;
  *_re = re = (double*)prealloc(re,4*sizeof(double));
...
void calc_rg(double **_rg)
{
  ...
  double M;
  *_rg = rg = (double*)prealloc(rg,4*sizeof(double));
...
void calc_rh(double **_rh)
{
  ...
  *_rh = rh = (double*)prealloc(rh,2*sizeof(double));
...
void calc_internal_dist(double **_idf) {
  ...
  double dx,dy,dz;
  double *idf=NULL;
  *_idf = idf = (double*)prealloc(idf,chain_length*sizeof(double));
...
void calc_bond_l(double **_bond_l) {
  ...
  double dx,dy,dz, tmp;
  double *bond_l=NULL;
  *_bond_l = bond_l = (double*)prealloc(bond_l,4*sizeof(double));
...
void calc_bond_dist(double **_bdf, int ind_n) {
  ...
  double dx,dy,dz;
  double *bdf=NULL;
  *_bdf = bdf = (double*)prealloc(bdf,chain_length*sizeof(double));
...
void init_g123()
{
  ...
  partCoord_g = (float *) prealloc(partCoord_g, 3*n_total_particles*sizeof(float));
  partCM_g = (float *) prealloc(partCM_g, 3*chain_n_chains*sizeof(float));
...
void calc_g1_av(double **_g1, int window, double weights[3]) {
  int i, j, p, t,k, cnt;
  double *g1=NULL;
  *_g1 = g1 = (double*)prealloc(g1,n_configs*sizeof(double));
...
void calc_g2_av(double **_g2, int window, double weights[3]) {
  ...
  double M;
  *_g2 = g2 = (double*)prealloc(g2,n_configs*sizeof(double));
...
void calc_g3_av(double **_g3, int window, double weights[3]) {
  ...
  double M;
  *_g3 = g3 = (double*)prealloc(g3,n_configs*sizeof(double));
...
void analyze_formfactor(double qmin, double qmax, int qbins, double **_ff) {
  ...
  *_ff = ff = (double*)prealloc(ff,(qbins+1)*sizeof(double));
  r_ij = (double*)prealloc(r_ij,chain_length*(chain_length-1)/2*sizeof(double));
...
void analyze_rdfchain(double r_min, double r_max, int r_bins, double **_f1, double **_f2, double **_f3) {
  ...
  *_f1 = f1 = (double*)prealloc(f1,r_bins*sizeof(double));
  *_f2 = f2 = (double*)prealloc(f2,r_bins*sizeof(double));
  *_f3 = f3 = (double*)prealloc(f3,r_bins*sizeof(double));
  cm = (double*)prealloc(cm,(chain_n_chains*3)*sizeof(double));
  min_d = (double*)prealloc(min_d,(chain_n_chains*chain_n_chains)*sizeof(double));
  for(i=0; i<r_bins; i++) {
    f1[i] = f2[i] = f3[i] = 0.0;
  }
...
void analyze_cwvac(int maxtau, int interval, double **_avac, double **_evac) {
    ...
    // result
    vac = (double*)prealloc(vac,(maxtau+1)*sizeof(double));
    *_avac = avac = (double*)prealloc(avac,(maxtau+1)*sizeof(double));
    *_evac = evac = (double*)prealloc(evac,(maxtau+1)*sizeof(double));
    
    // set up velarray
    varr = (double*)prealloc(varr,(3*(n_configs-1)*n_total_particles)*sizeof(double));
