...
/** use our own malloc which makes sure that pmalloc(0) returns NULL. */
#endif
/*@}*/
...
{
  il->max = size;
  il->e = (int *) pmalloc(sizeof(int)*il->max);
}
...
inline void realloc_intlist(IntList *il, int size)
{
  if(size != il->max) {
    il->max = size;
    il->e = (int *) prealloc(il->e, sizeof(int)*il->max);
  }
}
...
{
  dl->max = size;
  dl->e = (double *) pmalloc(sizeof(double)*dl->max);
}
...
inline void realloc_doublelist(DoubleList *dl, int size)
{
  if(size != dl->max) {
    dl->max = size;
    dl->e = (double *) prealloc(dl->e, sizeof(double)*dl->max);
  }
}
...
inline int lu_decompose_matrix(double **A, int n, int *perms) {
  ...
  double *scal = (double *)pmalloc(n*sizeof(double));
...
{
  int i,j;
  *grid = (double***)pmalloc(sizeof(double **)*dim[0]);
  if(*grid==NULL) return 0;
  for(i=0;i<dim[0];i++) {
    (*grid)[i] = (double**)pmalloc(sizeof(double *)*dim[1]);
    if((*grid)[i]==NULL) return 0;
    for(j=0;j<dim[1];j++) {
      (*grid)[i][j] = (double*)pmalloc(sizeof(double)*dim[2]);
...
{
  if (!left) {
    char *res = (char *)pmalloc(strlen(right) + 1);
    strcpy(res, right);
    return res;
  }
  else {
    char *res = (char *)prealloc(left, strlen(left) + strlen(right) + 1);
    strcat(res, right);
    return res;
  }
}
