...
void dp3m_init() {
  ...
  if (coulomb.Dbjerrum == 0.0) {       
    ...
    dp3m.send_grid = (double *) prealloc(dp3m.send_grid, sizeof(double)*dp3m.sm.max);
    dp3m.recv_grid = (double *) prealloc(dp3m.recv_grid, sizeof(double)*dp3m.sm.max);
    ...
    int ca_mesh_size = dfft_init(&dp3m.rs_mesh,
				 ...
				 &dp3m.ks_pnum);
    dp3m.ks_mesh = (double *) prealloc(dp3m.ks_mesh, ca_mesh_size*sizeof(double));
    
    for (n=0;n<3;n++)   
       dp3m.rs_mesh_dip[n] = (double *) prealloc(dp3m.rs_mesh_dip[n], ca_mesh_size*sizeof(double));
        ...
        P3M_TRACE(fprintf(stderr,"dipolar %d - interpolating (%d) the order-%d charge assignment function\n",
         ...
         dp3m.params.inter2 = 2*dp3m.params.inter + 1;
          ...
          for (i=0; i < dp3m.params.cao; i++) {
             ...
             dp3m.int_caf[i] = (double *) prealloc(dp3m.int_caf[i], sizeof(double)*(2*dp3m.params.inter+1));
...
double calc_surface_term(int force_flag, int energy_flag)
{
  ...
  double  *mx=NULL,*my=NULL,*mz=NULL;
     ...
     mx = (double *) pmalloc(sizeof(double)*n_local_part);
     my = (double *) pmalloc(sizeof(double)*n_local_part);
     mz = (double *) pmalloc(sizeof(double)*n_local_part);
     ...
     if (force_flag) {
          ...
          double *sumix = (double *) pmalloc(sizeof(double)*n_local_part);
          double *sumiy = (double *) pmalloc(sizeof(double)*n_local_part);
          double *sumiz = (double *) pmalloc(sizeof(double)*n_local_part);
...
void dp3m_realloc_ca_fields(int newsize)
{
  ...
  if (newsize < CA_INCREMENT) newsize = CA_INCREMENT;
   ...
   dp3m.ca_num = newsize;
   dp3m.ca_frac = (double *)prealloc(dp3m.ca_frac, dp3m.params.cao3*dp3m.ca_num*sizeof(double));
   dp3m.ca_fmp  = (int *)prealloc(dp3m.ca_fmp, dp3m.ca_num*sizeof(int));
...
void dp3m_calc_meshift(void)
{
  ...
  double dmesh;
     ...
     dp3m.meshift = (double *) prealloc(dp3m.meshift, dp3m.params.mesh[0]*sizeof(double));
...
void dp3m_calc_differential_operator()
{
  ...
  dmesh = (double)dp3m.params.mesh[0];
  dp3m.d_op = (double *) prealloc(dp3m.d_op, dp3m.params.mesh[0]*sizeof(double));
...
void dp3m_calc_influence_function_force()
{
  ...
  dp3m.g_force = (double *) prealloc(dp3m.g_force, size*sizeof(double));
...
void dp3m_calc_influence_function_energy()
{
  ...
  dp3m.g_energy = (double *) prealloc(dp3m.g_energy, size*sizeof(double));
  fak1  = dp3m.params.mesh[0]*dp3m.params.mesh[0]*dp3m.params.mesh[0]*2.0/(box_l[0]*box_l[0]);
